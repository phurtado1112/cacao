...
function generar_excell() {
    ...
    if (periodo != "" && anio != "" && usuario != "") {
        window.location.href = "http://localhost/cacao/index.php/contabilidad/reportes/estado_resultado/generar_excel/" + periodo + "/" + anio + "/" + usuario +"/"+moneda +"/"+presentacion;
...
function generar_pdf() {
    ...
    if (periodo != "" && anio != "" && usuario != "") {
        window.location.href = "http://localhost/cacao/index.php/contabilidad/reportes/estado_resultado/generar_pdf/" + periodo + "/" + anio + "/" + usuario +"/"+moneda +"/"+presentacion;
...
function mostrar_estado_resultado(array) {
    var presentacion = $("#presentacion").val();
    var moneda = $("#moneda").val();
    var periodo = $("#periodo").val();
    var anio = $("#anio").val();
    ...
    if (periodo != "" && anio != "" && usuario != "") {
        $.ajax({
            url: "http://localhost/cacao/index.php/contabilidad/reportes/estado_resultado/mostrar_reporte",
            type: 'POST',
            data: "periodo=" + periodo + "&anio=" + anio + "&usuario=" + usuario+ "&moneda="+ moneda +"&presentacion=" +presentacion+ "&simbolo=" + array[0] +"&cambio=" + array[1],
            success: function (data) {
                $("#resultado").html(data);
            },
            error:function (data) {
                alert("Error: no se pudo mostrar el resulatdo del reporte");
            }
        ...
        alert("Error:No se puede generar reporte");
    }
}
...
function buscar_tasa_por_fecha() {
    ...
    $.ajax({
        url: "http://localhost/cacao/index.php/contabilidad/reportes/estado_resultado/consulta_datos_moneda",
        type: 'POST',
        ...
        success: function (data) {
            if (data === 'vacio') {
                ...
                mostrar_estado_resultado(data.split("/")) ;
            }
        },
        error: function () {
